/*
* ACTIVIDAD 5 - Sentencias SQL Avanzadas y Vistas
* GRUPO 7 : Himar Rogriguez, Cristian Teruel
* Practica realizada en MySQL
*/
...
#TABLA DEPARTAMENTOS -> NUMDE(PK) : NUMCE(FK) ref NUMCE(centros), DEPDE(FK) ref NUMDE(departamentos)
...
#TABLA EMPLEADOS -> NUMEM(PK) : NUMDE(FK)(departamentos)
...
/*INSERTA LOS SIEGUIENTES DATOS EN LA TABLA DEPARTAMENTOS*/

/*
* Error al introdcuir el primer valor
* FK referencia a PK null (no creada), desactivamos restricciones para introducir
* Volvemos a activar
*/
...
/*INSERTA LOS SIGUIENTES DATOS EN LA TABLA EMPLEADOS*/
...
/* 1.Para cada departamento con presupuesto inferior a 35.000 €, hallar el nombre del Centro donde está ubicado y el máximo salario de sus empleados (si dicho máximo excede de 1.500 €).
 * Clasificar alfabéticamente por nombre de departamento*/
select D.nomde as nombDpto, C.nomce as nombCent, max(E.salar) as salarioMax
from departamentos D join empleados E on E.numde = D.numde
join centros C on D.numce = C.numce
where D.presu < 35
group by D.nomde
having salarioMax > 1500
...
select D.nomde as nombDpto, D2.nomde as dptoSup, D2.presu as presupuestoDptoSup
...
select D.nomde as nombDpto, round(avg(E.salar),2) as salMedioDpto, (select round(avg(salar),2) from empleados) as salMedioEmp
...
select D.nomde as nombDpto, count(E.numem) as numEmp, sum(E.salar) as sumaSalarios, sum(E.comis) as sumaComis, sum(E.numhi) as numHijos
from departamentos D join empleados E on E.numde = D.numde
where D.tidir = 'F'
group by nombDpto;
...
select E.extel, D.nomde, count(E.numem) as numEmpleados
from departamentos D join empleados E on D.numde = E.numde
where D.presu > 35
group by E.extel;

/* 6.Hallar por orden alfabético los nombres de los empleados
 * y su número de hijos para aquellos que son directores en funciones.*/
select D.nomde as nombDpto, E.nomem as nombEmp, E.numhi as numHijos
from departamentos D join empleados E on E.numem = D.direc
where D.tidir = 'F'
group by nombEmp asc;
...
select D.nomde as nombDpto, E.nomem as nombEmp
 ...
 * Muestra una segunda columna con los nombres de empleados para asegurarnos que realmente está a NULL.**/
select D.nomde as nombDpto, E.nomem as nombEmp
from empleados E right join departamentos D on E.numde = D.numde
group by nombDpto
...
select E.nomem as nombEmp, D.nomde as nombDep
from empleados E left join departamentos D on E.numde = D.numde
where D.numde is null;
/*Retorno esperado NORBERTO*/
...
/* 15.Muestra los departamentos que no tienen empleados y
 ...
 /*MySQL no admite FULL JOIN, se puede simular con left/right join y UNION*/
 select D.nomde as nombDep, E.nomem as nombEmp
 from departamentos D left join empleados E on D.numde = E.numde
 where E.numde is null
 UNION
 select D.nomde as nombDep, E.nomem as nombEmp
...
select E.nomem as nombEmp, D.nomde as nombEmp, C.nomce as nombCent
...
delete from departamentos where numde = (select numde from departamentos where nomde = 'NUEVO');

delete from empleados where numem = (select numem from empleados where nomem ='Norberto') and empleados.numde is null;